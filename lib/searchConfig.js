...
export function buildAugmentedSearchQuery({ query, fileType, siteRestrict } = {}) {
    let q = query.trim();
...
export function buildGoogleSearchUrl({ query, fileType, siteRestrict, dateRestrict, num } = {}) {
    const { apiKey, cx, countryRestrict, hl, safe, baseUrl, num: defaultNum } = GOOGLE_SEARCH_CONFIG;
    const url = new URL(baseUrl);
    const q   = buildAugmentedSearchQuery({ query, fileType, siteRestrict });
...
export function buildBingSearchRequest({ query, fileType, siteRestrict, dateRestrict, num } = {}) {
...
export function buildDuckDuckGoSearchRequest({ query, fileType, siteRestrict } = {}) {
...
export function resolveSearchProvider(priority = CUSTOM_SEARCH_PRIORITY) {
    ...
    const hasGoogle = Boolean(GOOGLE_SEARCH_CONFIG.apiKey && GOOGLE_SEARCH_CONFIG.cx);
...
export function resolveCustomLlmConfig(overrides = {}) {
    ...
    const url               = (overrides.url ?? overrides.baseUrl ?? CUSTOM_LLM_CONFIG.url).trim();
    const apiKey            = (overrides.apiKey ?? CUSTOM_LLM_CONFIG.apiKey).trim();
    const model             = (overrides.model ?? CUSTOM_LLM_CONFIG.model).trim();
    const systemPrompt      = (overrides.systemPrompt ?? CUSTOM_LLM_CONFIG.systemPrompt).trim() || DEFAULT_SYSTEM_INSTRUCTION;
