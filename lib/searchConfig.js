...
export function resolveCustomLlmConfig(overrides = {}) {
    ...
    const anthropicVersion  = (overrides.anthropicVersion ?? CUSTOM_LLM_CONFIG.anthropicVersion).trim() || "2023-06-01";
    const siteUrl           = (overrides.siteUrl ?? CUSTOM_LLM_CONFIG.siteUrl).trim();
    const appName           = (overrides.appName ?? CUSTOM_LLM_CONFIG.appName).trim() || "AdvocateHub";
    const missingFields     = [];
